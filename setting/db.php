...
class model_db {
	...
	public static function db_conect($db_config_array) {
		$db = new mysqli($db_config_array['default']['connection']['hostname'], $db_config_array['default']['connection']['username'], $db_config_array['default']['connection']['password'], $db_config_array['default']['connection']['database'], ini_get("mysqli.default_port"), ini_get("mysqli.default_socket"));
		if ($db->connect_error) {
	...
	// クエリ
	//-------
	public static function phpunitquery($query) {
		require(PATH . 'setting/db_config.php');
		$query_pattern = '';
		// DB接続
		$db = self::db_conect($db_config_array);
